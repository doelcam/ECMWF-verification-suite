...
c
      do jcla=1,ncla-1
         ii=(jcla*n)/(ncla-1)
         xi=amod(float(jcla*n),float(ncla-1))
         xi=float(jcla*n)/(ncla-1)-jcla*n/(ncla-1)
         if(ii.lt.1)then
           t2(jcla)=y(1)
         elseif(ii.ge.n)then
           t2(jcla)=y(n)
...
c     jcount :                                                         c
c
c     Now with added ignorance (DM 5/11/13)                            c
c----------------------------------------------------------------------c
c
...
c
              if(itag.eq.1)then
                ...
                do iens=1,nens
                   ...
                   if(xhlp(iens).eq.t_mod)num=num+0.5 ! ens'member forecasts event
                enddo !iens
                iprob=1+int(num+0.499999)
                if(yhlp.lt.t_era)then !event is verified
                  obs_occ_t(iprob)=obs_occ_t(iprob)+1.  !add obs_occ in corresp. prob'bin
                elseif(yhlp.gt.t_era)then !event is not verified
                  obs_noc_t(iprob)=obs_noc_t(iprob)+1.  !add obs_noc in corresp. prob'bin
                elseif(yhlp.eq.t_era)then !tie
                  obs_occ_t(iprob)=obs_occ_t(iprob)+0.5      !add obs_occ in corresp. prob'bin
                  obs_noc_t(iprob)=obs_noc_t(iprob)+0.5      !add obs_noc in corresp. prob'bin
                endif
              else
                num=0.
                do iens=1,nens
                ...
                if(yhlp.gt.t_era)then !event is verified
                  obs_occ_t(iprob)=obs_occ_t(iprob)+1.       !add obs_occ in corresp. prob'bin
                elseif(yhlp.lt.t_era)then !event is not verified
                  obs_noc_t(iprob)=obs_noc_t(iprob)+1.       !add obs_noc in corresp. prob'bin
                elseif(yhlp.eq.t_era)then !tie
                  obs_occ_t(iprob)=obs_occ_t(iprob)+0.5      !add obs_occ in corresp. prob'bin
                  obs_noc_t(iprob)=obs_noc_t(iprob)+0.5      !add obs_noc in corresp. prob'bin
...
c        unc(i)=ave_prob(i)*(1.0-ave_prob(i))
         unc(i)=totocc_rel(i)*(1.0-totocc_rel(i))
...
c..... Calculate the ignorance (D.MacLeod 5/11/13)
        ign(i) = 0.
	do iprob=1,nprob
            xpr=(iprob)/(nprob+1.)
	    ign(i) = ign(i)-obs_occ(i,iprob)*log(xpr)/log(2.)- 
     >            obs_noc(i,iprob)*log(1.-xpr)/log(2.)

        end do
        ign(i) = ign(i)/(sum(obs_occ_r(i,:))+sum(obs_noc_r(i,:)))
	
	
c       converting to ignorance skill score
	ign(i) = 1-ign(i)/(-pev*log(pev)/log(2.) -
     >	          (1-pev)*log(1-pev)/log(2.))
      enddo !i
c
      return
c
...
c.... Replacing g05caf and g05ccf from old NAG
      external g05saf,g05kgf
      INTEGER MSTATE,LSTATE,IFAIL
      PARAMETER (MSTATE=633)
      INTEGER STATE(MSTATE)
      PARAMETER (LSTATE=MSTATE)

c .... Initialising random number generator (non-reproducible)
      IFAIL = 0
...
c        write(*,*)'Scrambling ',jt
         do i=1,n
            do j=1,m
100	       IFAIL=0
               call g05saf(1,STATE,X1,IFAIL)
     	       WRITE(*,*) IFAIL
      
               l1=int(n*x1)+1

               if(l1.eq.i)goto 100
               call g05saf(1,STATE,X2,IFAIL)
               l2=int(m*x2)+1
               sh(i,j)=h(l1,l2)
               sh(i,j)=h(i,j)
            enddo
