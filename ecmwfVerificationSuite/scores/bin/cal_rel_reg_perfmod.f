...
c
      ...
      real, allocatable :: dat_ref_pm(:,:), dat_mod_pm(:,:,:)
...
c
      ...
      allocate (dat_ref_pm(nxny,nyear),                      stat=istat)
      allocate (dat_mod_pm(nxny,nyear,nens-1),               stat=istat)
      allocate (dat_ref_sin(nxny,1),                         stat=istat)
      allocate (dat_mod_sin(nxny,1,nens-1),                  stat=istat)
      allocate (dat_ref_ran(nxny,nyear),                     stat=istat)
      allocate (dat_mod_ran(nxny,nyear,nens-1),              stat=istat)
...
c                write(*,*)'Year ',iy
                 do i=1,nxny
                    ...
                    dat_ref_sin(i,1)=dat_mod(i,iy,1)
                    do iens=1,nens-1
                       dat_mod_sin(i,1,iens)=dat_mod(i,iy,iens+1)
                    enddo
                 enddo
                 call bsrelroc(nxny, iprb, 1, nens-1, nprob, pev,
...
c
              do iy=1,nyear
                 do i=1,nxny
                    dat_ref_pm(i,iy)=dat_mod(i,iy,1)
                    do iens=1,nens-1
                       dat_mod_pm(i,iy,iens)=dat_mod(i,iy,iens+1)
                    enddo
                 enddo
              enddo
              call bsrelroc(nxny, iprb, nyear, nens-1, nprob, pev,
     >           regwgt, latwgt, lsm, narea, nbin,
     >           dat_mod_pm, dat_ref_pm, thr_mod, thr_ref, itag,
     >           obs_occ, obs_noc, hr, fr, yhr, yfr, xprob,
     >           sample, yprob, bss_ful, bss_red, bss_inf,
     >           rel, res, bss_rel, bss_res,
     >           unc, totocc_rel, ave_prob, sd_prob,
...
c    >                  'Random values for sample ',ib,' are ',x,l
                      do i=1,nxny
                         ...
                         dat_ref_ran(i,iy)=dat_mod(i,l,1)
                         do iens=1,nens-1
                            dat_mod_ran(i,iy,iens)=dat_mod(i,l,iens+1)
                         enddo ! iens loop
                      enddo ! i loop
                   enddo ! iy loop
                   itag=2
