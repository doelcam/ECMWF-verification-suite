...
c     
      ...
      real, allocatable :: field_tmp(:,:), field_sig(:,:)
...
c     
         do idia=1,9
            do i=1,nxny
               ...
               if(boot.eq.1)then
                  if(idia.eq.7)field_sig(ilon,ilat)=bss_sig(i)
                  if(idia.eq.8)field_sig(ilon,ilat)=bssinf_sig(i)
                  if(idia.eq.9)field_sig(ilon,ilat)=rocss_sig(i)
               endif
            enddo
...
c     
     ...
     >           ' is a postscript file'
            form='(A,A,A,A,i1,A,i2,a)'
            if(nens.gt.9)form='(A,A,A,A,i2,A,i2,a)'
            if(nens.gt.99)form='(A,A,A,A,i3,A,i2,a)'
            WRITE(CTITG1,form)
...
c     CALL PSETC('output_ps_colour_model','RGB')
c     CALL PSETR('OUTPUT_PS_SCALE',0.70)
C     
            xlatn=90.
            xlats=-90.
            xlonw=60.
...
c     
     ...
     >           xmin,xmax,CTITG1,CTITG2,CTITG3,CTITG4,CTITG5,yofile,
...
c
      ...
      call psetr('subpage_top_position', SBPYL+1.5)
      call psetr('subpage_top_position', SBPYL+3.)
