...
c
      ...
      double precision, allocatable :: unc(:), totocc_rel(:), ign(:)
      ...
      real, allocatable :: bsssha_ran(:,:), rocss_ran(:,:), ign_ran(:,:)
      real, allocatable :: bss_sig(:), bssinf_sig(:), rocss_sig(:)
      real, allocatable :: ign_sig(:)
...
c
      ...
      integer iunit, ounit1, ounit2, ounit3, ounit4, ounit5, ounit6
      integer ounit7
      integer ounit11, ounit12, ounit13, ounit14
...
c
         ...
         write(form1,'(a,i2.2,a)')'(a',exptl,')'
         yofile=
     >'BSS_EXPT_YYYY-YYYY_MMDDTT_CVYY_II_PAR_LEV_THR_MM_BIN_AAA-EEE'
	 ...
	 if(boot.eq.1)then
	   write(yrfile(       1:       3),'(a3)')'BSI'
	   write(*,*)'open output file: ',yrfile(1:lena(yrfile))
	   kret=0
	   call pbopen(ounit12,yrfile,'w',kret)
	   if(kret.ne.0)then
             write(*,*)'Error in opening file: kret=',kret
             goto 997
	   endif
	 ...
	 write(*,*)'open output file: ',yofile
	 kret=0
	 call pbopen(ounit6,yofile,'w',kret)
	 if(kret.ne.0)then
           write(*,*)'Error in opening file: kret=',kret
           goto 997
	 endif
	 ...
	 if(boot.eq.1)then
	   write(yrfile(       1:       3),'(a3)')'ROC'
	   write(*,*)'open output file: ',yrfile(1:lena(yrfile))
	   kret=0
	   call pbopen(ounit13,yrfile,'w',kret)
	   if(kret.ne.0)then
             write(*,*)'Error in opening file: kret=',kret
             goto 997
	   endif
         ...
         write(yofile(         1:       3),'(a3)')'IGN'
         write(*,*)'open output file: ',yofile
         kret=0
         call pbopen(ounit7,yofile,'w',kret)
         if(kret.ne.0) then
             write(*,*)'Error in opening file: kret=',kret
             goto 997
         endif
...
c
     ...
     >      roc, rocsup, roclow, ipt, alpha, venv, jcount,ign)
c
...
c           write(*,'(i5,7e12.2)')i,
c    >         bss(i),bssinf(i),bssrel(i),bssres(i),bsssha(i),rocss(i),ign(i)
         enddo
         write(*,*)'Write BSS'
         do i=1,nxny
            psec4(i)=bss(i)
     ...
     >      ksec0, ksec1, ksec2, ksec3, ksec4, psec2, psec3)
         ...
         do i=1,nxny
            psec4(i)=rocss(i)
         enddo
         call writegrb(ounit6, nxny, psec4,
     >      ksec0, ksec1, ksec2, ksec3, ksec4, psec2, psec3)
         if(kret.lt.0)then
           write(*,*)'Error in closing file: kret=',kret
           goto 997
         ...
         write(*,*)'Write IGN skill score'
         do i=1,nxny
             psec4(i)=ign(i)
         enddo
         call writegrb(ounit7,nxny,psec4,
     >       ksec0, ksec1, ksec2, ksec3, ksec4, psec2, psec3)
         if(kret.lt.0)then
             write(*,*)'Error in closing file: kret=',kret
             goto 997
...
c    >             rocss(i),x_bot,x_top,rocss_sig(i)
                ...
                do ib=1,nsam
                    xarray(ib)=ign_ran(i,ib)
                enddo
                call edge(xarray,nsam,ntop,nbot,x_top,x_bot)
                ign_sig(i)=1.
                if((x_bot.lt.0.).and.(x_top.gt.0.))ign_sig(i)=0.
...
c
           ...
           call writegrb(ounit11, nxny, bss_sig,
     >        ksec0, ksec1, ksec2, ksec3, ksec4, psec2, psec3)
           write(*,*)'Write BSS infinite significance'
           call writegrb(ounit12, nxny, bssinf_sig,
     >        ksec0, ksec1, ksec2, ksec3, ksec4, psec2, psec3)
           write(*,*)'Write ROC significance'
           call writegrb(ounit13, nxny, rocss_sig,
     >        ksec0, ksec1, ksec2, ksec3, ksec4, psec2, psec3)
           write(*,*)'Write IGN significance'
           call writegrb(ounit14, nxny, ign_sig,
     >        ksec0, ksec1, ksec2, ksec3, ksec4, psec2, psec3)
         endif
      enddo !ievt
c
